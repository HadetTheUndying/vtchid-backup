 ...
 *
 * $Id: main.c,v 1.14 2004/06/12 11:40:49 hos Exp $
 *
 */
...

static NTSTATUS DDKAPI device_add(IN PDRIVER_OBJECT drv_obj,
                                  IN PDEVICE_OBJECT phys_dev_obj);
static VOID DDKAPI driver_unload(IN PDRIVER_OBJECT drv_obj);

static NTSTATUS DDKAPI irp_create(IN PDEVICE_OBJECT dev_obj, IN PIRP irp);
static NTSTATUS DDKAPI irp_close(IN PDEVICE_OBJECT dev_obj, IN PIRP irp);
static NTSTATUS DDKAPI irp_pnp(IN PDEVICE_OBJECT dev_obj, IN PIRP irp);
static NTSTATUS DDKAPI irp_power(IN PDEVICE_OBJECT dev_obj, IN PIRP irp);
static NTSTATUS DDKAPI irp_inter_ioctl(IN PDEVICE_OBJECT dev_obj, IN PIRP irp);
