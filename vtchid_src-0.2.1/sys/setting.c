 ...
 *
 * $Id: setting.c,v 1.3 2004/06/14 12:28:52 hos Exp $
 *
 */
...

/* default settings */
#define NUM_OF_DEF_ACTION (1 + 5 + 7 + 16 + (29 + 5 + 5 + 3) * 2)
...
static NTSTATUS get_registry_dword(IN PWSTR subkey, IN PWSTR name,
                                   OUT PDWORD val, IN DWORD def_val)
{
    PKEY_VALUE_PARTIAL_INFORMATION val_info;
    NTSTATUS nt_ret;
    ...
    if(val_info->Type != REG_DWORD || val_info->DataLength != sizeof(DWORD)) {
        ExFreePool(val_info);
        *val = def_val;
                                      ...
                                      IN PWSTR subkey)
{
    ...
    /* load DWORD setting */
    {
        struct {
            ...
            {L"auto light", &dev_ext->conf.auto_light, 1},
            {L"light slowly off speed", &dev_ext->conf.light_slowly_off_speed,
             20},
