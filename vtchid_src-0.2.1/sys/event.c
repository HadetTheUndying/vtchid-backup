 ...
 *
 * $Id: event.c,v 1.1 2004/06/12 11:40:49 hos Exp $
 *
 */
...
inline
static UCHAR conv_uchar_from_schar(SCHAR val)
{
    int v;
...
static UCHAR calc_ratio(UCHAR val, SHORT numer, SHORT denom, SHORT offset)
{
    ...
    if(! NT_SUCCESS(nt_ret)) {
        ...
        v = lrint((double)val * numer / denom + offset);
...
#define SET_DVAL(_c_, _v_) SET_AVAL(_c_, ((_v_) * 0xff))
    ...
    /* axis */
    SET_AVAL(AIM_X,    raw_in->ir.aiming_lever_x);
    SET_AVAL(AIM_Y,    raw_in->ir.aiming_lever_y);
    SET_AVAL(SIGHT_X,  conv_uchar_from_schar(raw_in->ir.sight_change_x));
    SET_AVAL(SIGHT_Y,  conv_uchar_from_schar(raw_in->ir.sight_change_y));
    SET_AVAL(ROTATION, conv_uchar_from_schar(raw_in->ir.rotation_lever));

    /* gear lever */
    SET_DVAL(GEAR_R, (raw_in->ir.gear_lever == -2));
    SET_DVAL(GEAR_N, (raw_in->ir.gear_lever == -1));
    SET_DVAL(GEAR_1, (raw_in->ir.gear_lever ==  1));
    SET_DVAL(GEAR_2, (raw_in->ir.gear_lever ==  2));
    SET_DVAL(GEAR_3, (raw_in->ir.gear_lever ==  3));
    SET_DVAL(GEAR_4, (raw_in->ir.gear_lever ==  4));
    SET_DVAL(GEAR_5, (raw_in->ir.gear_lever ==  5));

    /* tuner dial */
    SET_DVAL(TUNER_E,   (raw_in->ir.tuner_dial == 0x00));
    SET_DVAL(TUNER_ENE, (raw_in->ir.tuner_dial == 0x01));
    SET_DVAL(TUNER_NE,  (raw_in->ir.tuner_dial == 0x02));
    SET_DVAL(TUNER_NNE, (raw_in->ir.tuner_dial == 0x03));
    SET_DVAL(TUNER_N,   (raw_in->ir.tuner_dial == 0x04));
    SET_DVAL(TUNER_NNW, (raw_in->ir.tuner_dial == 0x05));
    SET_DVAL(TUNER_NW,  (raw_in->ir.tuner_dial == 0x06));
    SET_DVAL(TUNER_WNW, (raw_in->ir.tuner_dial == 0x07));
    SET_DVAL(TUNER_W,   (raw_in->ir.tuner_dial == 0x08));
    SET_DVAL(TUNER_WSW, (raw_in->ir.tuner_dial == 0x09));
    SET_DVAL(TUNER_SW,  (raw_in->ir.tuner_dial == 0x0a));
    SET_DVAL(TUNER_SSW, (raw_in->ir.tuner_dial == 0x0b));
    SET_DVAL(TUNER_S,   (raw_in->ir.tuner_dial == 0x0c));
    SET_DVAL(TUNER_SSE, (raw_in->ir.tuner_dial == 0x0d));
    SET_DVAL(TUNER_SE,  (raw_in->ir.tuner_dial == 0x0e));
    SET_DVAL(TUNER_ESE, (raw_in->ir.tuner_dial == 0x0f));

    /* buttons: right block */
    SET_DVAL(MAIN_WEAPON,     raw_in->ir.main_weapon);
    SET_DVAL(SUB_WEAPON,      raw_in->ir.sub_weapon);
    SET_DVAL(LOCK_ON,         raw_in->ir.lock_on);
    SET_DVAL(EJECT_CMD,       raw_in->ir.eject_cmd);
    SET_DVAL(COCPIT_HATCH,    raw_in->ir.cockpit_hatch);
    SET_DVAL(IGNITION,        raw_in->ir.ignition);
    SET_DVAL(START,           raw_in->ir.start);
    SET_DVAL(MLM_OPEN_CLOSE,  raw_in->ir.mlm_open_close);
    SET_DVAL(MLM_MAP_ZOOM,    raw_in->ir.mlm_map_zoom);
    SET_DVAL(MLM_MODE_SELECT, raw_in->ir.mlm_mode_select);
    SET_DVAL(SM_MODE_SELECT,  raw_in->ir.sm_mode_select);
    SET_DVAL(MM_ZOOM_IN,      raw_in->ir.mm_zoom_in);
    SET_DVAL(MM_ZOOM_OUT,     raw_in->ir.mm_zoom_out);

    /* buttons: center block */
    SET_DVAL(FSS,                raw_in->ir.fss);
    SET_DVAL(MANIPULATOR,        raw_in->ir.manipulator);
    SET_DVAL(LINE_COLOR_CHANGE,  raw_in->ir.line_color_change);
    SET_DVAL(WASHING,            raw_in->ir.washing);
    SET_DVAL(EXTINGUISHER,       raw_in->ir.extinguisher);
    SET_DVAL(CHAFF,              raw_in->ir.chaff);
    SET_DVAL(TANK_DETACH,        raw_in->ir.tank_detach);
    SET_DVAL(OVERRIDE,           raw_in->ir.override);
    SET_DVAL(NIGHT_SCOPE,        raw_in->ir.night_scope);
    SET_DVAL(F1,                 raw_in->ir.f1);
    SET_DVAL(F2,                 raw_in->ir.f2);
    SET_DVAL(F3,                 raw_in->ir.f3);
    SET_DVAL(WC_MAIN,            raw_in->ir.wc_main);
    SET_DVAL(WC_SUB,             raw_in->ir.wc_sub);
    SET_DVAL(WC_MAGAZINE_CHANGE, raw_in->ir.wc_magazine_change);
    SET_DVAL(CT_1,               raw_in->ir.ct_1);
    SET_DVAL(CT_2,               raw_in->ir.ct_2);
    SET_DVAL(CT_3,               raw_in->ir.ct_3);
    SET_DVAL(CT_4,               raw_in->ir.ct_4);
    SET_DVAL(CT_5,               raw_in->ir.ct_5);

    /* buttons: left block */
    SET_DVAL(SIGHT_BUTTON,            raw_in->ir.sight_change_button);
    SET_DVAL(FILT_CONTROL_SYSTEM,     raw_in->ir.filt_control_system);
    SET_DVAL(OXYGEN_SUPPLY_SYSTEM,    raw_in->ir.oxygen_supply_system);
    SET_DVAL(FUEL_FLOW_RATE,          raw_in->ir.fuel_flow_rate);
    SET_DVAL(BUFFER_MATERIAL,         raw_in->ir.buffer_material);
    SET_DVAL(VT_LOCATION_MEASUREMENT, raw_in->ir.vt_location_measurement);

    /* buttons: foot pedal */
    SET_AVAL(FOOT_PEDAL_LEFT,   raw_in->ir.foot_pedal_left);
    SET_AVAL(FOOT_PEDAL_CENTER, raw_in->ir.foot_pedal_center);
    SET_AVAL(FOOT_PEDAL_RIGHT,  raw_in->ir.foot_pedal_right);
...

{
    ...
    in_state->joy.throttle = pr_in->
                             joy.val[CONTROLLER_ACTION_JOY_CODE_THROTTLE];
