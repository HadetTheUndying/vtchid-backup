 ...
 *
 * $Id: prop_page_in.c,v 1.5 2004/06/13 13:03:47 hos Exp $
 *
 */
                                 ...
                                 prop_page_context_t *ctx)
{
    ...
    switch(nm->code) {
      case PSN_SETACTIVE:
          SetEvent(ctx->in.read_thread_run);
          SetWindowLong(hwnd, DWL_MSGRESULT, 0); /* set result of notify */
          return TRUE;

      case PSN_KILLACTIVE:
          ResetEvent(ctx->in.read_thread_run);
          SetWindowLong(hwnd, DWL_MSGRESULT, FALSE); /* set result of notify */
          return TRUE;
